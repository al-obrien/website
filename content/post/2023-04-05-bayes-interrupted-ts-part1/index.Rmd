...
tags: 
  ...
  - STAN
  - simulation
summary: 'This post is is the first of two parts. The focus here is on introducing a scenario and generating a simulated dataset, which is subsequently used for Bayesian interrupted time series modeling'
authors: []
...
> This post is is the first of two parts. The focus here is on introducing a scenario and generating a simulated dataset, which is subsequently used for Bayesian interrupted time series modeling in [part 2](/post/bayes-interrupted-ts-part2/).  
...
At this point you may be asking yourself, "how did we got down a rabbit hole on Fourier series?" Take comfort that the details, in this case, do not really matter. The main idea is we just want to decide on a pattern for seasonality and we are spoiled for choice. After we plot some of the available options using the helper functions it will become clearer. Below we look at four possible options.
...
There is a wealth of content on Interrupted Time Series as well as their applications in the R language, spanning articles, textbooks, and blogs. A selection that were referenced for part 1 and part 2 of this post are below:  
