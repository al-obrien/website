...
& age \sim Gamma((age_{shape}+ age\_parameter * sex), 1/age_{scale}) \\ \\
\end{align*}
$$
...
And finally, we need to assign a likelihood function. This is the distribution for *Disease X* based upon the relationship of age, sex, and location.

$$
\begin{align*}
& \textbf{Likelihood function}\,\text{(for prior predictive simulation)}\\
& disease \sim Binom(p, 1) \\
& logit(p) = a_{disease} + b_{age}(age) + b_{sex}(sex) + b_{urbanity}(urbanRural) \\\\
\end{align*}
$$
...
<strong>1. Metropolis-Hastings algorithm:</strong> an extension to the metropolis algorithm.  
<strong>1. Gibbs sampling:</strong> more efficient sampling by using *adaptive proposals* (`BUGS` and `JAGS` software use this method).  
<strong>1. Hamiltonian Monte Carlo (HMC):</strong> runs a (resource expensive) physics simulation to generate informative samples. HMC provides useful diagnostics but currently cannot sample discrete parameters (not easily at least). `STAN` uses this method and is considered harder to learn than `BUGS`.  
